...
class AnimationUtils(
    ...
    private val changeAnimationDescription: (resId: Int) -> Unit
) {
    ...
    private val temp = File(filesDir, "temp")
    private val output = File(filesDir, "/")
    ...
    fun emptyTemp(): Completable {
        ...
        return Completable.fromAction {
            if (temp.exists()) {
                Log.d(TAG, "temp folder recycling:")
    ...
    fun prepareEnvironment(): Completable {
        ...
        Log.d(TAG, "Preparing environment")

        return Completable.fromAction {
            File(filesDir, "output.gif").apply {
                if (exists()) {
                    Log.d(TAG, "existing output.gif deleted")
                    delete()
                }
            }
            File(filesDir, "output_i.gif").apply {
                if (exists()) {
                    Log.d(TAG, "existing output_i.gif deleted")
                    delete()
                }
            }
            File(filesDir, "palette.png").apply {
                if (exists()) {
                    Log.d(TAG, "existing palette.png deleted")
                    delete()
                }
            }
        }
